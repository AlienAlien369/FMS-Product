...
public class DriversController : ControllerBase
{
    ...
    public DriversController(DriverService driverService, DriverScorecardService scorecards,
        ApplicationDbContext db, ITenantContext tenant)
    {
    ...
    // ── Driver Scorecards ───────────────────────────────────────────────────
    // Scorecard actions live on the drivers surface (one controller per route
    // prefix — a second controller mounted at api/v1/drivers breaks the empty
    // list route). URLs are stable: /api/v1/drivers/{id}/scorecard etc.
    ...
    public async Task<ActionResult<ApiResponse<DriverScorecardDto>>> Recompute(Guid driverId)
    {
        ...
        await _scorecards.RecomputeAsync(driverId);
